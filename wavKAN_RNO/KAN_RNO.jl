...
function create_KAN_RNO(input_dim::Int64, output_dim::Int64, input_size::Int64, wavelet_names, layer_norm=false)
    ...
    out_layers_list = [KANdense(layer_output[i], layer_output[i+1], wavelet_names[i], base_activation, layer_norm) for i in 1:length(layer_output)-1]
    hid_layers_list = [KANdense(layer_hidden[i], layer_hidden[i+1], wavelet_names[i], base_activation, layer_norm) for i in 1:length(layer_hidden)-1]
